...
def generate_samples(model, num_samples, device, inpt_cond, sampler="dpmpp_2m", sigma_min=0.1, sigma_max=80):
    """
    ...
    with torch.no_grad():
        ...
        sigma_min = sigma_min
        sigma_max = sigma_max
        sigmas = sampling.get_sigmas_karras(n=50, sigma_min=sigma_min, sigma_max=sigma_max, device=device)  # Noise schedule

        # ✅ Choosing the correct sampler
        if sampler == "euler":
            samples = sampling.sample_euler(model, x, sigmas)
        elif sampler == "euler_ancestral":
            samples = sampling.sample_euler_ancestral(model, x, sigmas)
        elif sampler == "heun":
            samples = sampling.sample_heun(model, x, sigmas)
        elif sampler == "dpm_2":
            samples = sampling.sample_dpm_2(model, x, sigmas)
        elif sampler == "dpm_2_ancestral":
            samples = sampling.sample_dpm_2_ancestral(model, x, sigmas)
        elif sampler == "dpmpp_2m":
            samples = sampling.sample_dpmpp_2m(model, x, sigmas)
        elif sampler == "dpmpp_2m_sde":
            samples = sampling.sample_dpmpp_2m_sde(model, x, sigmas)
        else:
...
def extract_and_format_datetime(input_string):
    ...
    match = re.search(r'(\d{4}.\d{2}.\d{2}_\d{2}:\d{2}:\d{2})', input_string)
    if not match:
    ...
    formatted_datetime_str = datetime_str.replace('.', '-').replace('_', 'T')
...
def combine_images_outside_circle(image_tensor_1, image_tensor_2, center_x, center_y, radius):
    """
    ...
    height, width = image_tensor_1.shape[-2:]
    ...
    # Ensure the mask and image tensors have the same number of channels
    mask = mask.unsqueeze(0).expand_as(image_tensor_1)

    # Combine the images: inside the circle use image_tensor_1, outside use image_tensor_2
    combined_tensor = torch.where(mask, image_tensor_1, image_tensor_2)

    # Count the number of pixels inside the mask
    num_pixels_inside_mask = torch.sum(mask).item()

    return combined_tensor, num_pixels_inside_mask


def mask_outside_circle(image_tensor, center_x, center_y, radius, min_val):
    """
    ...
    masked_tensor = torch.where(mask.unsqueeze(0), image_tensor, torch.tensor(min_val))
...
def obtain_contour_all(image, thrs=30, min_contour_length=20, already_bin=None):
    # Binarize
...
def persistence_perd(FITS_data_path, img_size=256, day=1):
    ...
    if map_hmi.data.shape == (1024, 1024):
        # Adjust the metadata to correctly reflect the downscaled image
        header = map_hmi.meta.copy()
        downscale_factor = 1024 / 4096
    
        header['NAXIS1'] = 1024
        header['NAXIS2'] = 1024
    
        header['CDELT1'] = float(header['CDELT1']) / downscale_factor
        header['CDELT2'] = float(header['CDELT2']) / downscale_factor
    
        header['CRPIX1'] = float(header['CRPIX1']) * downscale_factor
        header['CRPIX2'] = float(header['CRPIX2']) * downscale_factor
    
        if 'RSUN_OBS' in header:
            header['RSUN_OBS'] = float(header['RSUN_OBS']) * downscale_factor

    map_hmi = sunpy.map.Map(map_hmi.data, header)
    ...
    new_height, new_width = img_size, img_size  # example sizes, adjust as needed
    ...
    # return (resized_tensor), (resized_tensor_out_warp)
